...
tags: [web, lfi, poison, "cron jobs", suid, "path hijacking"]     # TAG names should always be lowercase
image:
...
```
The line above is the very last line of the output. It indicates that `/home/archangel/secret/backup` tried to copy a non-existing file. The most important thing and the one that enables the last step of privilage escalation is the fact that the command used `cp`, and not its absolute path variant. So, the system will use the first executable named `cp` following the string of directories defined in `PATH`. A perfect scenario for **Path Hijacking**. On a side note, `strings` and  `ltrace` would also reveal the above-described vulnerability.
